...
namespace KashkeshetClient.MenuHandler.Options.ManagerOptions
{
    public class RemoveUserFromChatOption : IOptions
    {
        ...
        public RemoveUserFromChatOption(IContainerInterfaces containerInterfaces, IUser _user, ServerHandler serverHandler)
        {
        ...
        public void Operation()
        {
            ...
            _containerInterfaces.SystemOutput.Print("Please enter which user you want to remove to chat | CLICK --stop for stop type OR exit for exit option-- EXIT");
            ...
            {
                RequestType = MessageType.RemoveUserToChat,
                lsUsers = userNames,
                GroupName = groupName
            };
        ...
        private bool ValidateName(string allConnectedUser, string name, List<string> userNamesToAdd, ChatMessageModel chat)
        {
            ...
            {
                _containerInterfaces.SystemOutput.Print($"The user {name} not exist in Group memebers");
                return false;
            }
            if (userNamesToAdd.Contains(name))
            {
                _containerInterfaces.SystemOutput.Print($"The user {name} already in user to remove");
                return false;
            }
            if (name == _clientname)
            {
                _containerInterfaces.SystemOutput.Print($"You cannot remove yourself Group chat with yourself");
                return false; ;
            }
